  ...
  output$cumconf <- renderPlot({
    if (input$type=="Count") {
      ...
      if (input$scale != "Log") {
        ...
        temp = temp %>% group_by(Last.Update) %>% summarise(Confirmed = sum(Confirmed))
        p = ggplot(temp, aes(y=Confirmed, x=Last.Update)) + 
          geom_line(size = 1.5, colour = "#0078BF") + 
      ...
      if (input$scale == "Log") p = p + scale_y_continuous(trans='log10')
      if (length(values$State) > 0) {
    ...
    if (input$type == "Count") {
      ...
      if (nrow(temp) > 0 & any(temp$Deaths)!=0) {
        if (input$scale != "Log") {
          ...
          temp = temp %>% group_by(Last.Update) %>% summarise(Deaths = sum(Deaths))
          p = ggplot(temp, aes(y=Deaths, x=Last.Update)) + 
            geom_line(size = 1.5, colour = "#0078BF") + 
            scale_y_continuous(trans='log10') +
            #scale_y_continuous(limits = c(0, max(temp$Confirmed, na.rm = T)+10000))+
            scale_x_date(date_breaks = '2 days')+
