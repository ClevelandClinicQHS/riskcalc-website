  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        'Ethnicity',
        'Ethnicity',
        choices = c('Non-Hispanic', 'Hispanic', 'Unknown')
      ),
      selectInput('Gender', 'Gender', choices = c('Male', 'Female')),
      selectInput(
        'Smoking',
        'Smoking',
        choices = c('No', "Current Smoker", 'Former Smoker', "Unknown")
      ),
      numericInput(
        'BMI',
        'BMI',
      ...
      radioGroupButtons(
        ...
        choiceNames = c("ZIP", "GEOID at TRACT level"),
        choiceValues = c(1, 2)
      ),
      conditionalPanel(condition = "input.geo == 1",
                       textInput('ZIP', "5 digit ZIP code")),
      conditionalPanel(condition = "input.geo == 2",
                       textInput('GEOID', "11 digit GEOID at TRACT level")),
      selectizeInput(
        'risk',
        'Symptoms and risks',
        choices = c(
          "Exposed to COVID-19" = "Expose_to_covid_19",
          "Other family members with COVID-19" =
            "Family_covid_19",
          'Cough',
          "Fever",
          "Fatigue",
          "Sputum production" = "Sputum_production",
          "Flu like symptoms" = "Flu_like_symptoms",
          "Diarrhea",
      ...
      #materialSwitch("wbc", "WBC?", status="success", inline=T),
      materialSwitch(
        "platelets",
        "Platelets?",
        status = "success",
      ...
      materialSwitch("ast", "AST?", status = "success", inline =
                       T),
      ...
      # conditionalPanel(
      #   condition = 'input.wbc==true',
      #   numericInput('pre_testing_wbc', 'WBC (10^9 / L)', value = 5, min = 0, max = 50, step=1),
      #   bsTooltip("pre_testing_wbc", "Range: 0 - 50", "right", options = list(container = "body"))
      # ),
      conditionalPanel(
        condition = 'input.platelets==true',
        numericInput(
          'pre_testing_platelets',
          'Platelets (k/uL)',
          value = 150,
          min = 100,
          max = 500,
    ...
    mainPanel(
      ...
      shinyjs::hidden(div(
        ...
        wellPanel(
          awesomeCheckboxGroup(
            "followup",
            label = h3("Do you have any of these characteristics? Check all that apply"),
            choices = list(
              "A physician has suggested you need a COVID-19 test" = "1",
              "Recent travel history to high risk area" = "2",
              # "Known contact with a patient with COVID-19" = 3,
              "End stage renal disease" = "4",
              # "Diabetes" = 5,
              "Lung disease" = "6",
              # "HIV/AIDS" = 7,
              "Solid organ transplant" = "8",
              "None of the above" = "9"
            ),
            selected = NULL
          ),
      ...
      shinyjs::hidden(div(id = "thankyou", wellPanel(
        h3(
      ...
      p(
        ...
        " | ",
        a("Source Code", 
          href = "https://github.com/ClevelandClinicQHS/riskcalc-website/tree/main/COVID19", 
