...
fluidPage(theme = shinytheme("flatly"),
  ...
  if (confirm("This is a mailbox for reporting website errors to programmers for the risk calculator website.  If you have questions or concerns about a specific calculator, please use the calculator Author Contact Information found on the publication for that calculator.  Each calculator is documented by a specific publication with a corresponding author.")) {
    window.open("mailto:rcalcsupport@ccf.org");
          ...
          titlePanel('Predict risk of post-operative urinary retention in women undergoing pelvic reconstructive surgery'),
          sidebarLayout(
              sidebarPanel(id = "sidebar",
                           selectInput('TwoTeamSling', 'Laparoscopic sling', choices = c('No', 'Yes')),
                           selectInput('AnteriorRepair', 'Anterior vaginal repair', choices = c('No', 'Yes')),
                           selectInput('Sacro', 'Laparoscopic colposacropexy', choices = c('No', 'Yes')),
                           selectInput('VaginalHysterectomy', 'Vaginal hysterectomy', choices = c('No', 'Yes')),
                           selectInput('DM', 'Diabetes', choices = c('No', 'Yes')),
                           selectInput('OtherComorbidities', "Number of other comorbidities (excluding hypertension)", 
                                       choices = c(0:6))),
              mainPanel(
