...
shinyServer(function(input, output) {
  ...
  output$predTab <- render_gt(
    
    cbind(
      preds_lr()[, c(6, 11)],
      preds_dm()[, c(6, 11)],
      preds_any_rcr()[, c(6, 11)],
      preds_bcm()[, c(6, 11)]
    ) %>% 
      t() %>% 
      as.data.frame() %>% 
      add_column(
        Time = rep(c("5-year", "10-year"), 4),
        Outcome = rep(c("Local Recurrence", "Distant Recurrence", 
                        "Any Recurrence", "Breast Cancer Mortality"), each = 2)
      ) %>% 
      mutate(V1 = paste0(round(V1*100), "%"),
             V2 = paste0(round(V2*100), "%")) %>% 
      rename(
        "No PMRT" = V1,
        "PMRT" = V2
      ) %>% 
