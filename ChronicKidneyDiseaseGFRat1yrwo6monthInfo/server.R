...
res = data.frame(Result = c("Graft Survival 5-Years Post-Transplant","eGFR 1-Year Post-Transplant"), 
                 Outcome = c(paste("Probability: ", ifelse(round(prob1, 0) < 1, "<1", ifelse(round(prob1, 0) > 99, ">99", round(prob1, 0))), "%", sep = ""), paste(ifelse(round(prob2, 2) < 1, "<1", ifelse(round(prob2, 2) > 99, ">99", round(prob2, 2))), "mL/min per 1.73m", sep = "")))
